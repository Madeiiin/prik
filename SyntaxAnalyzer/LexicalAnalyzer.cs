...
namespace SyntaxAnalyzer
{
    public class LexicAnalyze
    {
        ...
        public LexicAnalyze(List<ServiceWord> serviceWords, List<Separators> separators, Form1 form)
        {
        ...
        public string StartLexicalAnalyzer(string programStr)
        {
            ...
            for (int i = 0; i < programStrArr.Length; i++)
            {
                ...
                {
                    _arrResult.Add($"[1, {CheckServiceWord(programStrArr[i])}]");
                    hasEnd = true;
                    break;
                }
                ...
                if (serviceWordId > -1 && serviceWordId!=(16) && serviceWordId != (17) && serviceWordId != (18))
                {
                    _arrResult.Add($"[0, {serviceWordId}]");
                
                }
                else if(serviceWordId > -1 && serviceWordId == (16))
                {
                    _arrResult.Insert(i - 5, $"[0, 16]");
                }
                else if(serviceWordId > -1 && serviceWordId == (17))
                {
                    _arrResult.Insert(i - 2, $"[0, 17]");
                }
                else if(serviceWordId > -1 && serviceWordId == (18))
                {
                    _arrResult.Insert(i - 1, $"[0, 18]");
                }
                ...
                {
                    _arrResult.Add($"[2, {identificatorId}]");
                }
                else if (numberId > -1)
                {
                    _arrResult.Add($"[3, {numberId}]");
                }
                else
                {
            ...
            if (!hasEnd)
            {
                _form.CatchError("Неверный конец программы. Требуется: end.");
            }
            _arrResult.Remove($"[0, 20]");
            _arrResult.Remove($"[0, 20]");
            _arrResult.Remove($"[0, 20]");
            _arrResult.Remove($"[0, 21]");
            _arrResult.Remove($"[0, 22]");
            _arrResult.Remove($"[0, 23]");
